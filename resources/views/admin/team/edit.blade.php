            ...
            <div class="tab-content">
                @foreach (config('translatable.locales') as $key => $locale)
                    <div class="tab-pane fade show @if ($key == 0) active @endif" id="{{ $locale }}" role="tabpanel">
                        ...
                        <div class="form-group">
                            <label>@lang('team.description') - @lang('general.'.$locale)<span class="text-danger"> * </span></label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="flaticon-edit"></i></span>
                                </div>
                                <input type="text" name="{{ $locale . '[description]' }}" placeholder="@lang('team.description')" class="form-control  pl-5 min-h-40px @error($locale . '.description') is-invalid @enderror" value="{{ old($locale . '.description', $data->translate($locale)->description) }}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label>@lang('team.img_alt') - @lang('general.'.$locale)<span class="text-danger"> * </span></label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="flaticon-edit"></i></span>
                                </div>
                                <input type="text" name="{{ $locale . '[img_alt]' }}" placeholder="@lang('team.img_alt')" class="form-control  pl-5 min-h-40px @error($locale . '.img_alt') is-invalid @enderror" value="{{ old($locale . '.img_alt', $data->translate($locale)->img_alt) }}">
                            </div>
                        </div>
